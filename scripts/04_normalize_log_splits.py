#!/usr/bin/env python3
"""Normalize the log-transformed splits."""
...
# Load the log-transformed splits
train = pd.read_csv('data/splits/train_log.csv')
val = pd.read_csv('data/splits/val_log.csv')
test = pd.read_csv('data/splits/test_log.csv')

# Columns to normalize (add time_elapsed_log)
cols_to_normalize = [
    ...
    'time_elapsed_log'  # Add the log-transformed target
]
...
train.to_csv('data/splits/train_log.csv', index=False)
val.to_csv('data/splits/val_log.csv', index=False)
test.to_csv('data/splits/test_log.csv', index=False)
...
print("Normalized log-transformed splits")
print(f"Scaler params saved to: data/processed/scaler_params_log.json")